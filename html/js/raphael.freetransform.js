...
Raphael.fn.freeTransform = function(subject, options, callback) {
	...
	function applyLimits(bbox) {
		...
		if ( ft.opts.scaleRange ) {
            ft.attrs.scale.x = Math.max(ft.attrs.scale.x,ft.opts.scaleRange[0]);
            ft.attrs.scale.y = Math.max(ft.attrs.scale.y,ft.opts.scaleRange[0]);
            ft.attrs.scale.x = Math.min(ft.attrs.scale.x,ft.opts.scaleRange[1]);
            ft.attrs.scale.y = Math.min(ft.attrs.scale.y,ft.opts.scaleRange[1]);
        }
	}
