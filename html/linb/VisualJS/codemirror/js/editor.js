...
// spans.
// linb
function makePartSpan(value, doc, type) {
  var text = value;
  if (value.nodeType == 3) text = value.nodeValue;
  else value = doc.createTextNode(text);
//linb  
  var scopeChars = Editor.Parser.scopeChars,
      bl= scopeChars && type && scopeChars.indexOf(type)!=-1;
  var span = doc.createElement(bl?"B":"SPAN");
  span.isPart = true;
...
//linb
      ...
      addEventHandler(document, "blur", function(){
        if(onBlur)onBlur();
      });
      var onDblclick = this.options.onDblclick;
      addEventHandler(document.body, "dblclick", function(){
        if(onDblclick){
            var start = select.selectionTopNode(self.container, true),
                end = select.selectionTopNode(self.container, false);
            if (start === false || end === false) return;
            onDblclick(start, end);
        }
      });
...
//linb
    ...
    highlightParens: function(jump, fromKey) {
      ...
      while (true) {
        ...
        if (found.status == "dirty") {
          ...
          if (jump && found.node)
            // linb
            select.focusAfterNode(/[\(\[\{]/.test(found.node.currentText)?found.node:found.node.previousSibling, this.container);
          break;
        }
      }
    },
    ...
    highlightDirty: function(force) {
      ...
      // modified by linb
      if(realwork && this.options.onRendered)
        this.options.onRendered(this, !start);
    ...
    highlight: function(from, target, cleanLines, maxBacktrack){
      ...
      function tokenPart(token){
        // linb
        var part = makePartSpan(token.value, self.doc, token.type);
