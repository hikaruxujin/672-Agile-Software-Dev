...
//input must be a valid object path
        ...
        iniComponents:function(){
            ...
            host.block6.append((new linb.UI.Input)
                .setHost(host,"iQ1")
                .setDirtyMark(false)
                .setLeft(73)
                .setTop(3)
...
(flag !==false?((t=linb.SC.get(key)) && (t.$event$||t.$linb$||t.$auto$) ?"":'<a href="javascript:;" onclick="return SPA.showCode(event,\''+key+'\');">&nbsp;&nbsp;&nbsp;&nbsp;['+linb.getRes('app.oCode')+']</a>'):"") +
                    '</h4>' +
        ...
        _clickForToggle:function(p,e,s,n){
            var f=SPA._clickForToggle,ff=SPA._clickForLoca, ics=SPA._iconPosMap, ths=linb(s);
            if(linb.Event.getSrc(e).nodeName=='A')return;
            var a=ths.next(),b,ta,t,id;
            if(linb.use(s).get(0).nodeName=='H4'){
                ta=a.query('textarea');
                if(!ta.isEmpty())
                    ta.each(function(o){
                        if(o.id!='code')return;
